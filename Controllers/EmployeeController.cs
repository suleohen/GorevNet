...
namespace GorevNet.Controllers{
    ...
    public class EmployeeController : Controller{
        ...
        public async Task<IActionResult> UpdateTaskStatus(int taskId, TaskStatus status, string comment = ""){
            var user = await _userManager.GetUserAsync(User);
            if (user == null){return Json(new { success = false, message = "Oturum süreniz dolmuş." });}
            var employee = _context.Employees.FirstOrDefault(e => e.Email == user.Email && e.IsActive);
            if (employee == null){return Json(new { success = false, message = "Çalışan bilgileri bulunamadı." }); }
        ...
        public async Task<IActionResult> AllMyTasks(){
            ...
            if (employee == null){
                TempData["ErrorMessage"] = "Çalışan bilgileri bulunamadı.";
                return RedirectToAction("Login", "Account");}
            var allTasks = _context.UserTasks
                .Where(t => t.AssignedUserId == employee.Id)
                .OrderByDescending(t => t.CreatedDate)
                .ToList();
            ViewBag.TotalTasks = allTasks.Count;
            ViewBag.PendingTasks = allTasks.Count(t => t.Status == TaskStatus.Beklemede);
            ViewBag.OngoingTasks = allTasks.Count(t => t.Status == TaskStatus.DevamEdiyor);
            ViewBag.CompletedTasks = allTasks.Count(t => t.Status == TaskStatus.Tamamlandı);
            ViewBag.OverdueTasks = allTasks.Count(t => t.DueDate < DateTime.Now && t.Status != TaskStatus.Tamamlandı);
            ViewBag.EmployeeName = employee.FullName;
        ...
        public async Task<IActionResult> ActiveTasks(){
            ...
            var activeTasks = _context.UserTasks
                .Where(t => t.AssignedUserId == employee.Id &&
                           (t.Status == TaskStatus.Beklemede || t.Status == TaskStatus.DevamEdiyor))
                .OrderByDescending(t => t.CreatedDate)
                .ToList();
            ViewBag.PendingTasks = activeTasks.Count(t => t.Status == TaskStatus.Beklemede);
            ViewBag.OngoingTasks = activeTasks.Count(t => t.Status == TaskStatus.DevamEdiyor);
            ViewBag.OverdueTasks = activeTasks.Count(t => t.DueDate < DateTime.Now);
            ViewBag.EmployeeName = employee.FullName;
        ...
        public async Task<IActionResult> TaskDetail(int id){
            ...
            if (employee == null){
                return RedirectToAction("Login", "Account");}
            var task = _context.UserTasks.FirstOrDefault(t => t.Id == id && t.AssignedUserId == employee.Id);
            if (task == null){
                TempData["ErrorMessage"] = "Görev bulunamadı veya bu görevi görüntüleme yetkiniz yok.";
                return RedirectToAction("AllMyTasks");}return View(task);}
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateTaskStatusAjax(int taskId, TaskStatus status, string comment = ""){
            var user = await _userManager.GetUserAsync(User);
            if (user == null){
            ...
            try{
                ...
                task.ModifiedDate = DateTime.Now;
                if (!string.IsNullOrWhiteSpace(comment)){
                    task.Comment = string.IsNullOrWhiteSpace(task.Comment)
                        ? comment
                        : task.Comment + $"\n[{DateTime.Now:dd.MM.yyyy HH:mm}] {comment}";}
                await _context.SaveChangesAsync();
                string statusText = status switch{
                    TaskStatus.Beklemede => "Beklemede",
                    TaskStatus.DevamEdiyor => "Devam Ediyor",
                    TaskStatus.Tamamlandı => "Tamamlandı",
                    _ => "Bilinmiyor"};
                string message = $"Görev durumu '{statusText}' olarak güncellendi.";
                return Json(new{
                    success = true,
                    message = message,
        ...
        private string GetStatusClass(TaskStatus status){
            return status switch{
                TaskStatus.Tamamlandı => "badge-success",
                TaskStatus.DevamEdiyor => "badge-warning",
                TaskStatus.Beklemede => "badge-info",
                _ => "badge-secondary"};}
        #endregion
        #region Profile
