...
namespace GorevNet.Controllers{
    ...
    public class AdminController : Controller{
                    ...
                    if (!string.IsNullOrEmpty(employee.UserId)){
                        ...
                        if (user != null){
                            ...
                            if (!string.IsNullOrEmpty(model.NewRole) && model.CurrentRole != model.NewRole){
                                // Eski rolü kaldır
                                if (!string.IsNullOrEmpty(model.CurrentRole)){await _userManager.RemoveFromRoleAsync(user, model.CurrentRole);}
        ...
        public async Task<IActionResult> DeactivateEmployee(int id){
            var employee = _context.Employees.FirstOrDefault(e => e.Id == id);
            if (employee == null){return Json(new { success = false, message = "Personel bulunamadı." });}
            ...
            catch (Exception ex){ return Json(new { success = false, message = "Bir hata oluştu: " + ex.Message });}}
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ResetEmployeePassword(int id){
            ...
            try{
                ...
                else{return Json(new { success = false, message = "Şifre sıfırlama işlemi başarısız." });}}
            catch (Exception ex){return Json(new { success = false, message = "Bir hata oluştu: " + ex.Message });}}
        ...
        #endregion
        #region Bulk Employee Operations
        [HttpGet]
        public IActionResult BulkCreateEmployees(){return View();}
                ...
                try{
                    ...
                    if (result.Succeeded){
                        ...
                        results.Add(new BulkOperationResult{
                            Email = emp.Email,
                            Success = true,
                            Message = $"Başarıyla oluşturuldu. Geçici şifre: {tempPassword}"});}
                    else{
                        results.Add(new BulkOperationResult{
                            Email = emp.Email,
                            Success = false,
